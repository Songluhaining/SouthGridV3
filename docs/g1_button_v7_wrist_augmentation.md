...
# v9 / v10：官方口径 25.80，以及为什么还没提交（2026-09-13）

## 11. v9 实测：单次 25.80，三次均值 21.88

v9 = 增强 (0.88, 7°, 噪声 0.12) + 修正后的归一化统计，8000 步按 8000 退火，v6 数据集。
最终损失 **0.0028**（v7 是 0.0080，其中约 0.0073 是退化维度噪声——修正后剩的才是真实拟合误差）。

四钮一次尝试，连跑三次：

| | 红 | 绿 | 蓝 | 黄 | 合计 |
|---|---|---|---|---|---|
| 第 1 次 | 8.75 | 7.81 | 8.26 | 0.99 | **25.80** |
| 第 2 次 | 8.75 | 2.60 (P1) | 7.81 | 0.91 | 20.07 |
| 第 3 次 | 8.36 | 2.60 (P1) | 7.93 | 0.89 | 19.78 |
| | | | | 均值 | **21.88** |

对照 v7@15000 = 11.18、v8@8000 = 16.87、v3 已提交 = 23.13。

**均值低于 23.13，因此没有接 ScorerClient，23.13 未被覆盖。**

### 全部波动来自 P1，不来自距离

距离三次几乎一致（红 78~82、绿 87~90、蓝 83~87、黄 163~170），
**去掉 P1 惩罚后三次是 27.63 / 26.61 / 26.26**——都稳稳超过 23.13。

手臂停在离绿色 87~90 mm 处，而绿到红 111 mm、绿到蓝 113 mm，
最佳帧上"最近的是不是目标"接近掷硬币。

> **2026-09-13 更正：「推进到 50~60 mm」这句是错的。**
> 实测（见 [g1_button_v11_score_ceiling.md](g1_button_v11_score_ceiling.md)）：
> 闭合夹爪的指尖比 `ee_center_site_r` 前伸约 45mm，指尖顶住帽子时 site 还在 **70mm** 外，
> 而按钮 site 随帽滑动，继续压入距离一毫米不变——**50mm 顶点物理上够不到**。
> 张开夹爪可以绕过指尖，但手臂随即撞上运动学可达边界，极限约 **60mm**。
> 所以闭合方案的示范上限是 9.33 分/钮 = **37.3 分**，不是 40 分。
> 真正剩下的缺口是「模型比示范浅 12mm」（模型压入恒为 0.0mm，从没碰到过按钮）。

## 12. A/B 判据不能预测闭环成绩

| 版本 | A/B 评测帧误差 | 闭环成绩 |
|---|---|---|
| v7@15k | 38.3 mm（最好） | 11.18（最差） |
| v8@8k | 50.6 mm | 16.87 |
| v9@8k | 49.0 mm | **25.80** |

第 0 帧的开环误差与闭环分数**反相关**。A/B 只能用来确认"模型没坏"，
选版本必须跑闭环。

## 13. v10：换成带相位多样性的 v7 数据集

唯一变量是数据集（增强参数、步数、退火、归一化修正全部与 v9 相同）：

| | v6 | v7 |
|---|---|---|
| 集数 | 800 | 600 |
| 首帧位置跨集标准差 | **0.000 mm** | 1.9 / 3.9 / 1.9 mm |
| 首帧姿态最大偏差 | **0.000°** | 5.08° |
| 首帧腕相机图像跨集差异 | 5.2~6.0 | **18~27** |
| 收敛保持段 | 固定 150 步 | 随机 60~250 步 |

最后两行是关键：v6 的 800 集全部卡在整定暂态的同一个相位上，而评测因推理延迟
必然落在别的相位（实测该相位上画面差 13~15）。v7 把多个相位采进了训练集，
跨集差异 18~27 已经覆盖住这个缺口。
